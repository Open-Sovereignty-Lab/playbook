...
```
site/src/content/docs/docs/introduction/module-0/index.md
                     ╰──┬──╯╰─┬─╯
                        │     │
...
| `src/content/docs/docs/introduction/index.md` | `/playbook/docs/introduction/` |
| `src/content/docs/docs/introduction/module-0/index.md` | `/playbook/docs/introduction/module-0/` |
| `src/content/docs/docs/introduction/module-0/glossary.md` | `/playbook/docs/introduction/module-0/glossary/` |
...
The five top-level content sections are:

- `introduction/` — the foundational framing (**Module 0**) and the three implementation **Phases** (e.g. `introduction/module-0/`, `introduction/phase-1/`). This is the orientation section and appears first in the nav.
- `guides/` — long-form guidance, plus the contributor reference page
- `templates/` — reusable legal/technical templates
- `tools/` — calculators and utilities
- `case-studies/` — real-world tribal examples

> **A folder needs an `index.md`** to have its own landing page (e.g. `introduction/module-0/index.md` is the Module 0 overview). Without one, the folder still works as a container but `/docs/introduction/module-0/` has no page of its own.

---

## Adding a page

1. **Pick the right section folder** under `src/content/docs/docs/` (`introduction/`, `guides/`, `templates/`, `tools/`, or `case-studies/`). For a multi-page topic, make a subfolder (like `module-0/`) with its own `index.md`.
...
The Module 0 and Phase groups work the same way — they keep a custom label on top of an autogenerated folder — and they're **nested inside the `Introduction` group**, because their content lives under `introduction/`:

```js
{
  label: "Introduction",
  items: [
    { slug: "docs/introduction" },         // the Introduction landing page
    {
      label: "Module 0: Sovereignty in the Digital Age", // pretty label we choose
      autogenerate: { directory: "docs/introduction/module-0" }, // pages come from the folder
    },
    // ...Phase 1–3 groups follow, same pattern
  ],
},
```

So to add page `0.5` to Module 0, you create `introduction/module-0/whatever.md` with `sidebar.order: 7` — and you're done. **You never edit `astro.config.mjs` for that.**
...
- **A folder's `index.md` is a page too**, and it shows up as the first item *inside* its group. Because the group already has a label (e.g. "Module 0: Sovereignty in the Digital Age"), give the index a short `sidebar.label` so the first item doesn't repeat the whole group name. That's why `introduction/module-0/index.md` uses:
...
### Why `Introduction` uses an explicit list, and the duplication gotcha

`Introduction` is the one group built from an explicit `items` list instead of a plain `autogenerate`, because it has to mix a loose page (the section landing) *with* the nested Module/Phase sub-groups shown above.

The reason it is **not** simply `autogenerate: { directory: "docs/introduction" }` is the trap that caused the original bug back when this content lived under `guides/`:

> ⚠️ **`autogenerate` recurses into _every_ subfolder.** `module-0/` and `phase-*/` live *inside* `introduction/`. Autogenerating `docs/introduction` would pull in the entire Module/Phase tree on its own — and combined with the explicit Module/Phase sub-groups, every one of those pages would appear twice.
>
> **Rule of thumb: each directory is owned by exactly one sidebar entry — never autogenerate a folder *and* also list it (or autogenerate one of its subfolders) elsewhere.**

So `Introduction` lists its landing page and the curated Module/Phase sub-groups explicitly, and never autogenerates `docs/introduction` itself. The other four sections — `Guides`, `Templates`, `Tools`, and `Case Studies` — are flat folders with no curated subfolders, so they each use a plain `autogenerate` and Just Work.
    ...
    └── content/
        └── docs/           # (1) the Starlight collection root — required name
            └── docs/       # (2) the "/docs" URL section — YOUR CONTENT GOES HERE
                ...
                ├── introduction/    # Module 0 + Phase material (orientation, first in nav)
                ├── guides/          # long-form guides + contributor reference
                ├── templates/
                ├── tools/
                └── case-studies/
```
